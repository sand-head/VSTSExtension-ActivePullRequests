...
import { PullRequestTableItem, PullRequestTableProps } from './PullRequestTable/PullRequestTable.models';
...
export const TabContents: React.FC<TabContentsProps> = ({ selectedTabId, pullRequests, ...props }) => {
  const tableProps: PullRequestTableProps = {
    pullRequests: selectedTabId === TabType.Drafts
      ? pullRequests.filter(pr => pr.isDraft && pr.author.id === props.userId)
